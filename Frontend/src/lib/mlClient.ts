...
export interface LlmIrrigationResponse {
  irrigation_schedule: Array<{ week: string; action: string; amount?: string; reason: string }>;
  notes?: string;
}

export async function getLlmIrrigationSchedule(body: IrrigationRequest, baseUrl?: string): Promise<LlmIrrigationResponse> {
  const apiBase = baseUrl || (import.meta as any).env?.VITE_ML_API_URL || 'http://127.0.0.1:5000';
  const res = await fetch(`${apiBase}/irrigation/llm`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(body),
  });
