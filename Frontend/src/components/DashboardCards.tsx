...
const DashboardCards = ({ forecastData, scenario, weeklyForecast, sowingDate, mlResponse }: DashboardCardsProps) => {
  const { t } = useI18n();
  ...
  return (
    <div className="flex flex-col gap-6" ref={dashboardRef}>
      ...
      <CropLifecycle
        ...
        cropCycle={mlResponse?.crop_cycle}
        featureImportances={mlResponse?.feature_importances}
        explanationText={mlResponse?.explanation_text}
      />
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {/* Source Toggle: Rule-based vs AI (Gemini) */}
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">Schedule Source</div>
            <div className="flex items-center gap-2">
              <Badge variant={!useLlm ? 'default' : 'secondary'} className="cursor-pointer" onClick={() => setUseLlm(false)}>Rule-based</Badge>
              <Badge variant={useLlm ? 'default' : 'secondary'} className="cursor-pointer" onClick={() => setUseLlm(true)}>AI (Gemini)</Badge>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {(useLlm && (forecastData.llmIrrigation?.schedule || []).length ? forecastData.llmIrrigation.schedule : forecastData.irrigationSchedule).map((week: any, index: number) => (
                <div
                  key={index}
                  className={`p-3 rounded-lg border ${week.action === 'Irrigate'
          ...
          {/* AI Notes */}
          {useLlm && forecastData?.llmIrrigation?.notes && (
            <div className="p-3 rounded-lg border bg-accent/40 text-sm">
              AI Notes: {forecastData.llmIrrigation.notes}
            </div>
          )}
