...
import { ResidueRecommendationResponse } from '@/services/residueService';

interface ResidueResultProps {
  residueData: ResidueRecommendationResponse | null;
}

const ResidueResult = ({ residueData }: ResidueResultProps) => {
  const { t } = useI18n();

  if (!residueData) {
    return (
      <Card className="h-full min-h-[400px] flex items-center justify-center border-dashed bg-muted/20 shadow-sm">
        <div className="text-center p-6 space-y-3">
          <div className="mx-auto w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center">
            <Leaf className="w-8 h-8 text-primary/50" />
          </div>
          <h3 className="text-lg font-medium text-muted-foreground">{t('no_recommendation_yet') || 'No Recommendation Yet'}</h3>
          <p className="text-sm text-muted-foreground/80 max-w-sm mx-auto">
            {t('fill_parameters_prompt') || 'Fill out the parameters on the left and click "Get Recommendation" to see personalized residue management strategies.'}
          </p>
        </div>
      </Card>
    );
