...
import { predictCrop, getIrrigationSchedule, getLlmIrrigationSchedule } from '@/lib/mlClient';
import { GeminiContext } from '@/lib/geminiClient';
...
const Dashboard = () => {
  ...
  const [llmIrrigation, setLlmIrrigation] = useState<{ schedule: any[]; notes?: string } | null>(null);
  ...
  const geminiContext: GeminiContext = {
    ...
    yieldPrediction: forecastData?.yieldPrediction,
    riskAssessment: forecastData?.riskAssessment,
    cropCycle: mlResponse?.crop_cycle,
    irrigationSchedule: forecastData?.irrigationSchedule,
    waterSavings: forecastData?.waterSavings,
    featureImportance: forecastData?.featureImportance,
  };

  // Debug: Log context to help troubleshoot
  console.log('Dashboard Gemini Context:', geminiContext);
    ...
    try {
      ...
      if (weeklyForecastData.length) {
        ...
        try {
          ...
          // Also try LLM-based schedule (non-blocking failure)
          getLlmIrrigationSchedule({
            crop_type: selectedCrop.charAt(0).toUpperCase() + selectedCrop.slice(1),
            sowing_date: sowingDate,
            weekly_forecast: weeklyForecastData,
            crop_cycle: ml,
            soil_profile: soilType ? { type: soilType, ph: soilPh ? Number(soilPh) : undefined, organicMatterPct: soilOrganicMatter ? Number(soilOrganicMatter) : undefined, drainage: soilDrainage || undefined } : undefined,
          }).then((resp) => setLlmIrrigation({ schedule: resp.irrigation_schedule || [], notes: resp.notes })).catch(() => setLlmIrrigation(null));
        } catch (e) {
        ...
        setForecastData({ ...mapped, llmIrrigation: llmIrrigation ? { schedule: llmIrrigation.schedule, notes: llmIrrigation.notes } : undefined });
